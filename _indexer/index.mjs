...
async function compileEntries(dir, name) {
  ...
  for await (const f of await fs.promises.opendir(folder)) {
    if (
      !f.isFile() ||
      path.extname(f.name) != ".yaml" ||
      !f.name.endsWith(".option.yaml")
    ) {
      continue;
    }

    const entry = YAML.parse(
      await fs.promises.readFile(path.join(folder, f.name), "utf8")
    );
...
for await (const f of walk(".")) {
  ...
  const index = YAML.parse(
    await fs.promises.readFile(path.join(f.dir, INDEX_FILENAME), "utf8")
  );
  if (!indexValid(index)) {
